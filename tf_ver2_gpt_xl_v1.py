...
class GPT_Network(tf.keras.Model):
    ...
    def base_decode(
        self, step, x_emb_pos, curr_state, 
        ...
        layer_out = [
            tf.expand_dims(dec_inputs, axis=0)]
        neg_infty = -1.0e9
    ...
    def call(
        ...
        if prev_inputs is None:
            prev_inputs = tf.zeros(
                ...
                name="prev_zeros", dtype=tf.float32)
        
        if curr_state is None:
            curr_state = tf.zeros(
                [self.n_layers, 
                 batch_size, 1, self.hidden_size], 
                name="curr_zeros", dtype=tf.float32)
        ...
        dec_outputs = self.base_decode(
            decode_step, x_embed_pos, curr_state, 
            prev_inputs, x_dec_embed, p_keep, p_reg=p_reg)
        
        # Get the next state. #
        next_state  = tf.reduce_mean(
            dec_outputs[:-1], axis=2, keepdims=True)
        
        # Project to vocabulary. #
        dec_logits = tf.tensordot(
            dec_outputs[-1, :, :, :], 
            self.p_decoder, [[2], [0]])
        return dec_logits, dec_outputs, next_state
    ...
    def infer(self, x_infer, dec_steps):
        ...
        for step in range(dec_steps):
            ...
            tmp_outputs = self.base_decode(
                decode_step+1, x_embed_pos, curr_state, 
                prev_inputs, x_infer_emb, p_keep, p_reg=1.0)
            
            if (step+1) % self.win_length == 0:
                prev_inputs = tmp_outputs[1:]
                curr_state  += tf.reduce_mean(
                    tmp_outputs[:-1], axis=2, keepdims=True)
